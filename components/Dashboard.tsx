    ...
    const fetchProjectsAndAnalysis = async () => {
      ...
      if (isOnline) {
          try {
            ...
            // Using gemini-2.5-flash-lite-latest for fast AI responses
  ...
  return (
    <div className="p-4 md:p-8 space-y-6">
      <section className="glass-card p-6 rounded-[2.5rem] relative overflow-hidden">
        <div className="flex items-center space-x-2 mb-3"><div className={`w-8 h-8 ${isOnline ? 'bg-brand-indigo' : 'bg-gray-600'} rounded-xl flex items-center justify-center text-white shadow-lg`}><BrainCircuit size={18} /></div><span className={`text-[10px] font-black ${isOnline ? 'text-brand-indigo' : 'text-gray-400'} uppercase tracking-widest`}>AI Technical Briefing</span></div>
        {isLoading ? <SkeletonLoader count={2} /> : <p className="text-slate-200 text-sm leading-relaxed font-bold">"{aiBriefing}"</p>}
      </section>

      <section className="glass-panel text-white p-6 rounded-[2.5rem] relative overflow-hidden">
        <div className="flex items-center space-x-2 mb-4"><Navigation size={14} className={isOnline ? 'text-emerald-400' : 'text-gray-500'} /><span className={`text-[10px] font-black uppercase tracking-widest ${isOnline ? 'text-emerald-400' : 'text-gray-500'}`}>{isOnline ? 'Grounding Active' : 'Grounding Offline'}</span></div>
        ...
        {nearbyResources.length > 0 ? (
          ...
          <button onClick={findNearbySuppliers} className={`w-full py-4 rounded-2xl font-black text-xs uppercase transition-colors ${isOnline ? 'bg-white text-gray-900 hover:bg-slate-200' : 'bg-gray-700 text-gray-400 cursor-not-allowed'}`} disabled={isFindingResources || !isOnline}>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {aiProjects.map((p) => (
              <div key={p.id} onClick={() => handleProjectSelect(p.id)} className="glass-card p-5 rounded-[2rem] transition-all cursor-pointer group hover:border-brand-cyan/50 hover:ring-4 hover:ring-brand-cyan/10 hover:scale-[1.02] hover:bg-white/5">
                ...
                <div className="text-[10px] text-slate-400 mt-1 uppercase font-bold tracking-widest">{p.address}</div>
                <div className="mt-4 bg-white/5 p-3 rounded-xl text-[10px] font-bold text-slate-300">{p.aiSummary}</div>
              </div>
            ))}
          </div>
        )}
