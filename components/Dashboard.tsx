...
import { ChevronRight, MapPin, PlusCircle, Search, BrainCircuit, TrendingDown, Sparkles, Info, CheckCircle2, AlertTriangle, ShieldCheck, BookOpen, Focus, Navigation, FilePlus2, Eye } from 'lucide-react';
import { GoogleGenAI, Type } from "@google/genai";
import { useAppContext } from '../context/AppContext';
import { getProjects, AIProjectData, getProjectById, Project } from '../data/mockApi';
...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (<div className={`inline-flex items-center px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${colorClasses}`}><div className={`w-2 h-2 rounded-full mr-2 ${dotClasses}`} /><span>{status}</span></div>);
};


const Dashboard: React.FC = () => {
  const { setSelectedProjectId, setActiveTab } = useAppContext();
  ...
  return (
    <div className="p-4 md:p-8 space-y-6">
      ...
      <section className="pb-4">
        <div className="flex items-center justify-between mb-4 px-1"><h2 className="font-black text-gray-900 tracking-tight">AI Prioritized Portfolio</h2><button onClick={() => setActiveTab('new-project')} className="flex items-center space-x-2 bg-blue-50 text-blue-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border border-blue-100"><FilePlus2 size={12}/><span>New</span></button></div>
        {isLoading ? <SkeletonLoader height="150px" count={3} borderRadius="2rem"/> : (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {aiProjects.map((p) => (
              <div key={p.id} onClick={() => handleProjectSelect(p.id)} className="bg-white p-5 rounded-[2rem] border border-gray-100 shadow-sm transition-all cursor-pointer group hover:border-blue-500 hover:ring-4 hover:ring-blue-50">
