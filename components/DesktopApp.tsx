...
type ProjectTab = 'details' | 'billing' | 'logs' | 'photos' | 'equipment' | 'tic-sheet';
...
const DesktopApp: React.FC = () => {
    ...
    const renderMainContent = () => {
        switch(mainView) {
            ...
            case 'projects':
                if (!selectedProject) return <div className="p-8 text-center text-blue-500">Select a project to begin.</div>;
                return (
                    <div className="flex flex-col h-full">
                        <header className="p-6 border-b border-white/10 flex-shrink-0">
                            ...
                            <div className="mt-4 flex items-center space-x-1 border border-white/10 rounded-lg p-1 bg-slate-950/30 w-min overflow-x-auto no-scrollbar">
                                <ProjectTabButton label="Project Details" active={projectTab==='details'} onClick={()=>setProjectTab('details')} />
                        ...
                        <div className="flex-1 overflow-y-auto bg-slate-900">
                           {projectTab === 'details' && <ProjectDetails />}
                           {projectTab === 'billing' && <div className="p-4 md:p-8 bg-slate-900"><Billing /></div>}
                           {projectTab === 'logs' && <div className="p-4 md:p-8 bg-slate-900"><DryingLogs project={selectedProject} onOpenAnalysis={()=>{}} /></div>}
                           {projectTab === 'photos' && <div className="p-4 md:p-8 bg-slate-900"><PhotoDocumentation project={selectedProject} onStartScan={()=>{}} /></div>}
                           {projectTab === 'equipment' && <div className="p-4 md:p-8 bg-slate-900"><EquipmentManager project={selectedProject} /></div>}
    ...
    return (
        <div className="h-screen w-full bg-slate-950 text-blue-200 flex">
                    ...
                    <div className="flex-1 overflow-y-auto">
                        {projects.map(p => (
                            <button key={p.id} onClick={() => handleSelectProject(p.id)} className={`w-full text-left p-4 border-l-4 transition-colors ${selectedProjectId === p.id ? 'bg-brand-cyan/10 border-brand-cyan' : 'border-transparent hover:bg-white/5'}`}>
                                ...
                                <p className="text-xs text-blue-400">{p.address}</p>
            ...
            <main className="flex-1 flex flex-col bg-slate-900">
                 <header className="flex items-center justify-end p-4 bg-slate-950/30 border-b border-white/10 relative z-10">
                    <div className="relative">
                        <button onClick={() => setIsRoleMenuOpen(!isRoleMenuOpen)} className="flex items-center space-x-2 text-sm font-bold text-blue-300 bg-white/5 px-3 py-2 rounded-lg hover:bg-white/10">
                            <UserCircle size={18} />
...
const DesktopNavButton: React.FC<{ active: boolean; onClick: () => void; icon: React.ReactNode; label: string }> = ({ active, onClick, icon, label }) => (
    <button onClick={onClick} className={`flex flex-col items-center justify-center space-y-1 py-3 rounded-xl w-full transition-colors ${active ? 'bg-brand-cyan/10 text-brand-cyan' : 'text-blue-400 hover:bg-white/10'}`}>
      {icon}
...
const ProjectTabButton: React.FC<{ label: string, active: boolean, onClick: () => void }> = ({ label, active, onClick}) => (
    <button onClick={onClick} className={`px-4 py-2 text-xs font-bold rounded-md transition-all whitespace-nowrap ${active ? 'bg-brand-cyan text-slate-900 shadow-md' : 'text-blue-300 hover:bg-white/10'}`}>
        {label}
    </button>
);
